...
- `lib/default.nix` is a pure function `{ lib, hosts }` returning the derived views (`names`, `addresses`, `byRole`, `byArch`) plus `errors`, the validator.
...
2. Update the host table in `README.md`.
   Nothing generates it, so it drifts silently.
3. Run `command make check`.

Adding a new **field** or allowed value touches more still:

- `lib/default.nix`: the `fields`, `architectures`, or `roles` list, and `hostErrors` if the field needs a shape check beyond membership.
- `modules/flake/hosts.nix`: the `flake.hosts` option type is `attrsOf (attrsOf str)`, so any non-string value fails there even when `lib.errors` is empty.
- `README.md`: both the field table and the usage table.
